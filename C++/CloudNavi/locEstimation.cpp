...
{
    std::cout << std::endl;
    std::cout << "Usage: " << program_name << " file_name[gt_file] file_name[est_file] path[gt_cloud] path[est_cloud] file_name[results]" << std::endl;
...
//            std::cout << "\tNo. of correspondence remain is " << int (correspondences_remain->size ()) << std::endl;
        ...
        outfile << most_similar_cloud << "\t" << most_similar_num << "\t" << est_locX_string << "\t"
                << est_locY_string << "\t" << most_similar_locX << "\t" << most_similar_locY << endl;
    }
    outfile.close();
